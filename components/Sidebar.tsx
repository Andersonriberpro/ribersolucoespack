  ...
  return (
    <aside className="w-64 bg-slate-900 dark:bg-slate-950 text-slate-300 flex flex-col h-screen sticky top-0 flex-shrink-0 border-r border-transparent dark:border-slate-900 transition-colors">
      ...
      <div className="p-4 mt-auto border-t border-slate-800 dark:border-slate-900 space-y-2">
        <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-800/50 dark:bg-slate-900/50">
          <img src="https://picsum.photos/40/40?random=1" className="w-10 h-10 rounded-full border-2 border-indigo-500 flex-shrink-0" alt="Avatar" />
          <div className="overflow-hidden flex-1">
            <p className="text-sm font-semibold text-white truncate">{userName}</p>
            <p className="text-[10px] text-slate-500 truncate uppercase font-bold tracking-tighter">Administrador</p>
          </div>
        </div>
        <button
          onClick={onLogout}

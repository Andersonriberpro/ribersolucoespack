...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, onLogout, userName, isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 dark:bg-slate-950 text-slate-300 flex flex-col h-screen flex-shrink-0 border-r border-transparent dark:border-slate-900 transition-all duration-300 lg:sticky lg:top-0 lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
